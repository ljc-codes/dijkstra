...
public class Dijkstra {
  ...
  private Map<Vertex,Boolean> knownMap;
  ...
  public Dijkstra() {
    ...
    knownMap = new HashMap<Vertex,Boolean>();
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
    double inner = (Math.pow(ux - vx,2)+ Math.pow(uy - vy,2));
  ...
  public void computeAllEuclideanDistances() {
    for (Vertex x : vertexNames.values()){
      for (Edge e : x.adjacentEdges){
        ...
        if (e != null){
          ...
          e.distance = computeEuclideanDistance(fromx,fromy,tox,toy);
        }
      }
    }
  }
  ...
  public void doDijkstra(String s) {
    ...
    vertexList = new LinkedList<Vertex>(vertexNames.values());

    Vertex last = vertexNames.get(s);

    for (Vertex x : vertexNames.values()){
      x.distance = Double.POSITIVE_INFINITY;
      x.known = false;
      knownMap.put(x,false);
    }
    ...
    while (knownMap.containsValue(false)){
      min = findMin(vertexList);
      rmv = vertexList.remove(min);
      min.known = true;
      knownMap.put(min,true);

      for (Edge e : min.adjacentEdges){
        adj = e.target;
        if (!adj.known){
          cost = e.distance;
          if (min.distance + cost < adj.distance){
            adj.distance = min.distance + cost;
            adj.prev = min;
          }
        }
      }
    ...
    // Find min Method for Linked List of Vertex;

    public Vertex findMin(LinkedList<Vertex> verList){
      Vertex minVertex = verList.getFirst();
      for (Vertex x : verList){
        if ((minVertex.distance < x.distance == false) && x.known == false){
          minVertex = x;
        }
      }
      return minVertex;
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    list.addFirst(vertexNames.get(s));
    System.out.println("LIST: "  + list);

    // Convert Vertex List;

    LinkedList<Edge> finalEdgeList = new LinkedList<Edge>();
    double distance = 0;

    for (int i = 0; i < list.size() - 1; i++){
      for (Edge e : list.get(i).adjacentEdges){
        if (e.target.equals(list.get(i + 1))){
          distance = e.distance;
        }
      }
      finalEdgeList.add(new Edge(list.get(i),list.get(i+1),distance));
    }

    System.out.println("Lui G. was here");
